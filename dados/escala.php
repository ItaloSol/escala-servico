...
if (isset($_POST['escala'])) {
 ...
 while($escala = $query_escala->fetch(PDO::FETCH_ASSOC)) {     
  ...
  if ($servicoescala == 'NENHUM') {
     ...
     $Escala_Nome_Militar[$a] = 'NINGUEM';
     $Escala_Tipo[$a] = 'PRETA';
     $Escala_Posto[$a] = 'NENHUM';
   //         //           //
   // SUBISTITUIÇÃO
   $Subistituido_S1_Id[$a] = 0;
   $Subistituido_S2_Id[$a] = 0;
   $Subistituido_Quest[$a] = $subistituicao;
 
   // DADOS DOS MILITARES NA ESCALA
         $Escala_Militar_Id[$a] = 0;
         $Escala_Militar_Antiguidade[$a] = 0;
         $Escala_Militar_Nome[$a] = 'NINGUEM';
         $Escala_Militar_Servico[$a] = 0;
         $Escala_Militar_Graduacao[$a] = 0;
         $Escala_Militar_Atividade[$a] = 0;
         $Escala_Militar_Data_Ultimo_Sv[$a] = 0;
         $Escala_Militar_Id_Impedimento[$a] = 0;
         $Escala_Militar_Data_ultimo_Sv_Red[$a] = 0;
   //           //          //    
  }else{
      // DADOS DA ESCALA 
      $Escala_Id[$a] = $idescala;
 ...
 while($escala = $query_escala->fetch(PDO::FETCH_ASSOC)) {     
  ...
  if($servicoescala == 'NENHUM'){
      // DADOS DA ESCALA 
      $Desc_Escala_Id_Posto[$a] = 0;
      $Desc_Escala_Id[$a] = $idescala;
      $Desc_Escala_Data[$a] = $dataescala;
      $Desc_Escala_Nome_Militar[$a] = 'NINGUÉM';
      $Desc_Escala_Tipo[$a] = 'PRETA';
      $Desc_Escala_Posto[$a] = 'NENHUM';
      //         //           //
      // SUBISTITUIÇÃO
      $Desc_Subistituido_S1_Id[$a] = 0;
      $Desc_Subistituido_S2_Id[$a] = 0;
      $Desc_Subistituido_Quest[$a] = $subistituicao;
      //    //    //    //
      // DADOS DOS MILITARES NA ESCALA
      $Desc_Escala_Militar_Id[$a] = 0;
      $Desc_Escala_Militar_Antiguidade[$a] = 0;
      $Desc_Escala_Militar_Nome[$a] = 'NINGUÉM';
      $Desc_Escala_Militar_Servico[$a] = 0;
      $Desc_Escala_Militar_Graduacao[$a] = 'NENHUM';
      $Desc_Escala_Militar_Atividade[$a] = 0;
      $Desc_Escala_Militar_Data_Ultimo_Sv[$a] = 0;
      $Desc_Escala_Militar_Id_Impedimento[$a] = 0;
      $Desc_Escala_Militar_Data_ultimo_Sv_Red[$a] = 0;
      //           //          //    
  } else {
    // DADOS DA ESCALA 
...
if(!isset($dataescala)){
  $escala_nao_existe = 1;
  $Desc_Escala_Data[0] = date('Y-m-d');
}
if(!isset($data_ultimo_sv)){
  $Desc_Escala_Militar_Data_Ultimo_Sv[0] = date('Y-m-d');
}
