...
$query_data_ultimo_sv = $conexao->prepare("SELECT * FROM militares WHERE id = '$id'");//SELECIONA A TABELA DE MILITARES
$query_data_ultimo_sv->execute(); //EXECUTA TABELA
      ...
      while($ultimosv = $query_data_ultimo_sv->fetch(PDO::FETCH_ASSOC)  ){
		...
		$antiguidade_bruta = $ultimosv['antiguidade'];
	  }
	  $antiguidade = $antiguidade_bruta - ($antiguidade_bruta * 2);
...
if(!empty($id)){
	$result_usuario = "UPDATE militares SET servico = 'LICENCIADO', atividade = '0', data_ultimo_sv = '$hoje' ,antiguidade = '$antiguidade' WHERE id='$id'";
	$resultado_usuario = mysqli_query($conn, $result_usuario);
	if(mysqli_affected_rows($conn)){
		$_SESSION['msg'] = "<h2 style='color:green;'>Usuário Licenciado com sucesso</h2>";
		header("Location: ../gerenciamento/deletargerencia.php");
	}else{
		
		$_SESSION['msg'] = "<h2 style='color:red;'>Erro o usuário não foi Licenciado com sucesso</h2>";
		header("Location: ../gerenciamento/deletargerencia.php");
	}
}else{	
