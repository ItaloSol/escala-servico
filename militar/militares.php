...
<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */ if ($adm) : ?>
   ...
   <option value="militaresgrad.php" >Graduacao</option>
                                ...
                                while ($gradussacao = $graduacoes->fetch(PDO::FETCH_ASSOC)) {
                                    $id_graduacao_graduasa[$qtd_grad] = $gradussacao['id_graduacao'];
                                    $nome_graduacao_graduasa[$qtd_grad] = $gradussacao['nome_graduacao'];
                                    $qtd_grad++;
                                }
    ...
    function diasDatas($data_inicial,$data_final) {
        ...
        if (isset($_POST['filtro'])) {
         ...
         //  echo "SELECT $campos FROM militares m INNER JOIN graduacoes g ON g.id_graduacao = m.graduacao INNER JOIN cursos c ON m.fk_cursos = c.id_curso  WHERE $where_ $Oerden  ";
            $query_data_ultimo_sv = $conexao->prepare("SELECT $campos FROM militares m INNER JOIN graduacoes g ON g.id_graduacao = m.graduacao INNER JOIN cursos c ON m.fk_cursos = c.id_curso  WHERE $where_  $Oerden  "); //SELECIONA A TABELA DE MILITARES
        } else {
            if (isset($_GET['Ord'])) {
                if ($_GET['Ord'] == 'AASC') {
