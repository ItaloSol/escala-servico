...
<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */ if ($adm) : ?>
    ...
    <header class="hidden-print">
        ...
        <h2  class="hidden-print" style="color:black">Escolha um Militar Para Exibir o Historico<h2>
                <a href="listahistoricoexterno.php"> <b>Internos</b>
    ...
    <form class="hidden-print" method="POST" action="listahistorico.php">
    ...
    if (isset($_POST['submit'])) {
        ...
        if (isset($data2)) {
            if (isset($where)) {
                $where = $where . ' AND a.data <= "' . $data2 . '" ';
            } else {
                $where = ' a.data <= ' . $data2 . ' ';
            }
        }
        $idcon = $_POST['id_usuario0'];
        if (isset($where)) {
            if ($idcon != '') {
                $sql = "SELECT * FROM escala a 
                                        INNER JOIN militares m ON a.fk_militar = m.id 
                                     
                                        INNER JOIN posto p ON a.fk_posto = p.id_posto INNER JOIN graduacoes g ON m.graduacao = g.id_graduacao WHERE a.fk_militar = $idcon AND $where
                                        ORDER BY a.data DESC";
            } else {
                $sql = "SELECT * FROM escala a 
                                    INNER JOIN militares m ON a.fk_militar = m.id 
                                 
                                    INNER JOIN posto p ON a.fk_posto = p.id_posto INNER JOIN graduacoes g ON m.graduacao = g.id_graduacao WHERE $where
                                    ORDER BY a.data DESC";
            }
        } else {
            $sql = "SELECT * FROM escala a 
                                    ...
                                    INNER JOIN posto p ON a.fk_posto = p.id_posto INNER JOIN graduacoes g ON m.graduacao = g.id_graduacao WHERE a.fk_militar = $idcon
                                    ORDER BY a.data DESC";
        }
        // echo $sql;
                                    ...
                                    <table class="mdl-data-table mdl-js-data-table mdl-cell mdl-cell--12-col">
                                        ...
                                        <tbody>
                                            ...
                                            while ($user_data = mysqli_fetch_assoc($result)) {
                                                ...
                                                echo '<td>
                                        <a  href="deletarsv.php?id=' . $user_data['id_escala'] . '&tipo=' . $user_data['tipo'] . '&posto='. $user_data['tipo_folga'].'&id_posto='.$user_data['id_posto'].'" data-confirm="Tem certeza de que deseja excluir o item selecionado?" class="btn btn-sm btn-danger" href="#">
