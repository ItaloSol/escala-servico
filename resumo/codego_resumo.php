...
<div class="card" style="background-size: cover; background-image: url(../img/camuflado.jpg); backdrop-filter: blur(5px);">
    ...
    <div class="card-body">
        <div class="table-responsive text-nowrap">
            <table  class="table borrado table-bordered">
                <thead>
 ...
 while($escala = $query_escala->fetch(PDO::FETCH_ASSOC)) {   
    ...
    if($servicoescala == 'NENHUM'){
        $ontem[$a] = $escala['data']; 
        $idescala = $escala['id_escala'];
       $idpostoe = 0;
       $dataescala = $escala['data'];
       $nome_de_guerraescala = 'NINGUÉM';
       $postoescala = 'NENHUM';
       $idmilitar = 0;
       $antiguidade = 0;
       $servico = 'PRETA';
                ...
                <tbody>
                    ...
                    if ($Qtd_Prevista[0] != 0) {
                        do {
                            ...
                            //
                            //  IMPEDIMENTO DIARIO 
                            $Contar_Preta = $conexao->prepare("SELECT * FROM impedimentos i INNER JOIN militares m ON m.id = i.fk_militares WHERE '$dataprevista' BETWEEN  i.data_inicio AND i.data_final");
                            ...
                            if (isset($Externa_Id_Militar)) {
                                if (!isset($Externa_Id_Militar[0])) {
                                    $Impedido_Externa = '(' . $Externa_Id_Militar[0] . ')';
                                } else {
                            ...
                            $Data_folga_troca = date('Y-m-d', strtotime('-' . $Qtd_Prevista[3] . 'day', strtotime($Data_fo)));
                            ...
                            if (isset($Troca_Sv)) {
                                if (!isset($Troca_Sv[1])) {
                                    $Impedido_Troca_Lista = '(' .  $Troca_Sv[0] . ')';
                                } else {
                            ...
                            $query_postos = $conexao->prepare("SELECT * FROM posto WHERE status_posto = '1' AND tipo_folga != 'externo' AND prioridade != '-1' AND data_posto LIKE '%$dia_semanaPt%' ORDER BY prioridade ASC");
                            ...
                            $Folga_Externa = date('Y-m-d', strtotime('-' . $Qtd_Prevista[4] . 'day', strtotime($dataprevista)));
                            // '<br>';
                            $Folga_Missao = date('Y-m-d', strtotime('-' . $Qtd_Prevista[1] . 'day', strtotime($dataprevista)));

                            $Folga_Escalado_Externo = date('Y-m-d', strtotime('-' . $Qtd_Prevista[4] . 'day', strtotime($dataprevista)));
                         ...
                         //   echo "DATA: ".$exibedata." <br> Impedido_Troca_Lista: ".$Impedido_Troca_Lista ."<br> Previstos_Escalados: ". $Previstos_Escalados. "<br> Impedido_Dia: ". $Impedido_Dia."<br> Impedido_Externa: " . $Impedido_Externa. "<br> ";
                            while ($Qtd_Postos > $busca) {
                                ...
                                if ($Postos_Servico[$busca] == 'PRETA') {
                                    $Folga_Obrigatoria = date('Y-m-d', strtotime('-' .  $Qtd_Prevista_Graduacoes[$n] . 'day', strtotime($dataprevista)));
                                    //  echo $exibedata .' => ';
                                    ...
                                    // AND m.escalado_externo < '$Folga_Escalado_Externo' AND m.escalado_externo_red < '$Folga_Escalado_Externo' AND id NOT IN $Impedido_Troca_Lista  AND id NOT IN $Impedido_Dia 
                                    // AND id NOT IN  $Previstos_Escalados AND m.missao_red < '$Folga_Missao' AND m.missao < '$Folga_Missao' AND id NOT IN $Impedido_Externa AND m.servico != '$Diferente_De[$busca]'  
                                    // AND m.fk_cursos = '$Postos_Curso[$busca]' AND m.servico != 'LICENCIADO' AND m.graduacao IN $Postos_Graduacao[$busca] ORDER BY m.$Postos_Folga[$busca] ASC , m.antiguidade DESC LIMIT $Pagina_Atual , $Postos_Qtd[$busca] <br>";
                        ...
                        <td><ul class="list-unstyled users-list m-0 avatar-group  align-items-center">';
                            ...
                            while ($Percorrer_Postos < $Qtd_Postos) {
                                ...
                                if(isset($Postos_Qtd[$Percorrer_Postos])){
