...
if ($Escala == false) {
  ...
  <div class="row">
    ...
    <div class="col-6">
      <div style="left:55%;" class="abrir btn btn-danger" onclick="acaomodal()">Definir data de servico</div>
    </div>

    <div class="col-6">
      <div onclick="acaosem()" style="left:55%;" class="abrir btn rounded-pill btn-warning">Ver Militares não escalados
      </div>
    </div>
  </div>
  ...
  <div class="modal ">
    ...
    if(isset($escala_nao_existe)){
      echo '<br><b style="font-size: 30px;">É necessário Definir Manualmente o primeiro serviço no sistema</b><br><br><a class="btn btn-primary" style=" font-size: 30px; " href="../gerenciamento/definirinsert.php">Ir Para o menu de Definição Manual!</a>';
    } else {
      if (!isset($Escalados_Id)) {
        if ($Desc_Escala_Data[0] != $Data_Ontem) {
          echo '<b>Defina o serviço da data anterio!</b>';
        } else {
          echo '<b style=" margin-top: 40%; font-size: 30px; display: flex; align-items: center; justify-content: center; ;" >Definir essa data sem nenhum militar?</b>';
          echo '<a href="definicao.php?dt=' . $Data_Selecionada . '" style=" margin-left: 30%; width: 115px; height: 98px; font-size: 50px; display: flex; align-items: center; justify-content: center; background-color: yellow; border-radius: 50%;">Sim</a>';
    ...
    <form method="POST" action="definicao.php">
      <input type="hidden" id="data" name="data"
        value="<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */echo $Data_Selecionada ?>">
      <input type="hidden" id="TIPO" name="tipo"
        value="<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */echo $Tipo_Sv_Previsto ?>">
      <input type="hidden" id="qtd" name="qtd"
        value="<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */echo $Total_De_Militares_Escalados ?>">
      <input type="hidden" id="tipo_sgt" name="tipo_sgt"
        value="<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */echo $Tipo_Sgt ?>">
      <?php
      $a = 0;
      while ($a < $Id_Escalados) { ?>
        <input type="hidden" name="id_posto_a<?= $a ?>" id="id_posto_a<?= $a ?>"
          value="<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */print_r($Id_Dos_Postos_Escalados[$a]); ?>">
        <input type="hidden" name="id_definir<?= $a ?>" id="id_definir<?= $a ?>"
          value="<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */print_r($Escalados_Id[$a]); ?>">
    ...
    <b style="font-size: 20px;">
      <?= $exibedata ?> Definir data de serviço para os militares:
    </b>
...
?>
  <?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */if ($Escala == true) { ?>
    ...
    <div class="modal" style="z-index: 1;">
      ...
      <br>
      <b style="font-size: 20px;">
        <?= $exibedata ?> Definir serviço para os militares:
      </b>
      <form action="definicao.php" method="POST">
        <input type="hidden" id="data" name="data"
          value="<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */echo $Data_Selecionada ?>">
        ...
        <input type="hidden" id="tipo_sgt" name="tipo_sgt"
          value="<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */echo $Tipo_Sgt ?>">
        <input type="hidden" id="num_escalado" name="num_escalado"
          value="<?php /* SISTEMA DE ESCALA DE SERVIÇO CRIADO POR SD ÍTALO SOL SCLOCOO *DANTAS*  */echo $Total_De_Militares_Escalados ?>">
