...
function sobreavisoPreta(){
    ...
    while($posto = $query_postos->fetch(PDO::FETCH_ASSOC)) {
        ...
        $posto_graduacao_ = $posto['posto_graduacao'];
        ...
        if($posto_graduacao_ == '19'){
            ...
            ////
            $SobreAviso = $conexao->prepare("SELECT * FROM militares m INNER JOIN graduacoes g ON m.graduacao = g.id_graduacao WHERE m.graduacao IN $posto_graduacao AND servico != 'LICENCIADO' AND m.atividade = '1' ORDER BY m.$tipo_folga ASC, m.antiguidade DESC");
           ...
           $SobreAvisos_Graduacao[$qtd_postos] = $Array_Nome;
    ...
    while($a < $SobreAvisos_Qtd_Posto[$b]){
       ...
       echo '<td>'.$SobreAvisos_Graduacao[$b][$a] .'</td>';
       echo '<td>'.$SobreAvisos_Nome_De_Guerra[$b][$a] .'</td>';
       echo '<td>'.date('d/m/Y', strtotime($SobreAvisos_Data_Sv[$b][$a])) .'</td></tr>';
       }
       $a++;
       if($a == 15){
        break;
