...
if ($Dia_Semana == 's�b' or $Dia_Semana == 'sab' or $Dia_Semana == 'Sab') {
    ...
    while($posto_sobre = $query_postos_sobre->fetch(PDO::FETCH_ASSOC)) {
        ...
        $curso_postos = $posto_sobre['curso'];
        if($posto_graduacao_ == '19'){
            ...
            ////
            $SobreAviso = $conexao->prepare("SELECT * FROM militares m INNER JOIN graduacoes g ON m.graduacao = g.id_graduacao WHERE m.graduacao IN $posto_graduacao AND m.fk_cursos = '$curso_postos' AND m.servico != 'LICENCIADO' AND m.atividade = '1' AND m.id NOT IN $Impedido_Dia ORDER BY m.$tipo_folga ASC, m.antiguidade DESC");
...
?>
  ...
  <div class="modalinfo" style="z-index: 1;" onclick="fecharinfo()">
  <div onclick="fecharinfo()" class="fechar">X</div>
...
<div class="modalinfored" style="z-index: 1;" onclick="fecharinfored()">
  <div onclick="fecharinfored()" class="fechar">X</div>
